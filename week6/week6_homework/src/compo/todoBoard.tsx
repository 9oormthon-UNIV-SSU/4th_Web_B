...
type FilterState = '전체' | '완료' | '미완료';

export default function TodoBoard({items, setItems}: TodoBoardProps) {
  const [filterState, setFilterState] = useState<FilterState>('전체')
  const filterList : FilterState[] = ['전체', '완료', '미완료'];
  const emptyText : Record<FilterState, string> = {
    '전체' : '할 일이 없습니다',
    '완료' : '완료한 일이 없습니다',
    '미완료' : '미완료된 일이 없습니다'
  } 
  ...
  let list_rendering : React.JSX.Element[] = [];
